	...
	if directory.open(p_from) == OK:
		...
		while (file_name != "" && file_name != "." && file_name != ".."):
			if directory.current_is_dir():
				...
				var f_p = p_to + "/" + file_name
				directory.copy(p_from + "/" + file_name, f_p)
				var file_data = load(f_p)
				if (file_data is LevelData):
					for i in range(0, file_data.task.size()):
						if (file_data.task[i] != null):
							file_data.task[i].resource_path = file_data.task[i].resource_path.replace("res://", Global.dataFolderPreset + "://")
					ResourceSaver.save(file_data.resource_path, file_data)
...
static func addNewThings(newThings):
	if newThings and (newThings.things is ObjetRes or newThings.things is Monnaie):
		var path
		if newThings.things is ObjetRes:
			path = Global.dataFolderPreset + "://player_data/inventaire/"
		elif newThings.things is Monnaie:
			path = Global.dataFolderPreset + "://player_data/monnaie/"
